...
const PieItem = ({ pie, stores }) => {
  ...
  return (
    <div className="card mr-2 mb-2 pie-card">
      <div className="position-absolute text-center raleway" style={styleRating}>
        {store.rating}
      </div>

      <div className="card-body">
        <h2 className="card-title pie-card-title raleway">{pie.displayName}</h2>
        <h6 className="card-subtitle raleway mb-2 text-muted">{store.displayName}</h6>
        <p className="card-text raleway">
          {pie.priceString} ({getQuantityDescription(pie.quantity)})
        </p>
        <p className="card-text raleway">
          {getStoreFullAddress(store)}<br/>
